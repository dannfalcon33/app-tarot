...
import { MINOR_ARCANA_BASTOS } from "@/data/minorArcana";

export default function BastosMinorArcanaPage() {
  const router = useRouter();
  const [currentIndex, setCurrentIndex] = useState(0);

  const currentCard = MINOR_ARCANA_BASTOS[currentIndex];

  const goNext = () => {
    setCurrentIndex((prev) => (prev + 1) % MINOR_ARCANA_BASTOS.length);
  };

  const goPrev = () => {
    setCurrentIndex((prev) =>
      prev === 0 ? MINOR_ARCANA_BASTOS.length - 1 : prev - 1
    );
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center py-8 px-4">
      <div className="w-full max-w-6xl mx-auto">
        ...
        <h1 className="text-3xl md:text-4xl font-bold text-center text-white mb-3 drop-shadow-lg">
          Arcanos Menores de Bastos
        </h1>
        <p className="text-center text-purple-200 mb-8 max-w-2xl mx-auto">
          Explora los Arcanos Menores de Bastos, relacionados con la energía,
          la pasión y la acción. Usa las flechas o selecciona una carta para
        ...
        <div className="flex flex-col items-center gap-8">
          ...
          <div className="w-full flex flex-col items-center">
            ...
            <div className="max-w-2xl text-center text-purple-100 bg-black/40 border border-purple-500/40 rounded-2xl p-6 shadow-xl">
              ...
              <p className="text-sm uppercase tracking-widest text-purple-300 mb-4">
                Arcano Menor de Bastos {currentCard.id}
              </p>
          ...
          <div className="w-full max-w-4xl">
            <div className="flex flex-wrap justify-center gap-2 md:gap-3">
              {MINOR_ARCANA_BASTOS.map((card, index) => (
                <button
                  key={card.id}
